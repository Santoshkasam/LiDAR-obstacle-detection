...
struct KdTree
{
	...
	void insertHelper(Node **node, int depth, std::vector<float> point, int id)
	{
		...
		else 
		{
			...
			// Compares Xs or Ys depending on cd, and inserts in left or right child accordingly
	...
	void searchHelper(Node *node, int depth, std::vector<float> target, float distanceTol, std::vector<int> &ids)
	{
		if(node != NULL)
		{
			...
			if ((target[depth%2]-distanceTol) < node->point[depth%2]) searchHelper(node->left, depth+1, target, distanceTol, ids);
			if ((target[depth%2]+distanceTol) > node->point[depth%2]) searchHelper(node->right, depth+1, target, distanceTol, ids);
		}
